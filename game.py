...
pointarray = [[0 for x in range(500)] for x in range(2)]
playerone = ""
playertwo = ""
gamecount = 0
...
class Player:
    ...
    def new_generation_agents(self):
        ...
        if pointarray[0][gamecount] != 0 and pointarray[1][gamecount] != 0:
            gamecount += 1
        ...
        try:
            ...
            sys.stdout.write(" %.2e\n" % fitness)
            sys.stdout.flush()
            self.fitness.append(int(fitness))
            if playerone == "":
                playerone = self.playerFile
                pointarray[0][gamecount] = (fitness)
            elif playertwo == '':
                playertwo = self.playerFile
                pointarray[1][gamecount] = (fitness)
            elif playerone == self.playerFile:
                pointarray[0][gamecount] = (fitness)
            else:
                pointarray[1][gamecount] = (fitness)
        except Exception as e:
    ...
    def run(self,player1File, player2File,show_games, save_games, visResolution=(720,480), visSpeed='normal'):
        ...
        for game in range(1,self.nGames+1):
            ...
            if survivorCount[0]>survivorCount[1]:
                player1 += 1
                sys.stdout.write(' player1:%d, player2:%d, draw:%d, won by %s (blue) %d-%d\n' % (player1, player2, draw, self.players[0].name,survivorCount[0],survivorCount[1]))
                
            elif survivorCount[1]>survivorCount[0]:
                player2 += 1
                sys.stdout.write(' player1:%d, player2:%d, draw:%d, won by %s (red) %d-%d\n' % (player1, player2, draw, self.players[1].name,survivorCount[1],survivorCount[0]))
                
            else:
                draw += 1
                sys.stdout.write(' player1:%d, player2:%d, draw:%d, drawn %d-%d\n' % (player1, player2, draw, survivorCount[0],survivorCount[1]))
            sys.stdout.flush()
...
x1 = pointarray[0]
x2 = pointarray[1]
turn = np.array(np.arange(0,500)).tolist()
plt.figure(0)
plt.plot(turn, x1) 
plt.plot(turn, x2)
...
y = pointarray[0]
points_incurve = 15
model = np.poly1d(np.polyfit(x, y, points_incurve))
line = np.linspace((np.min(x)-1), (np.max(x)+1), 1000000)
plt.plot(x, y)
